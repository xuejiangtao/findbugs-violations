...
/**
 * Parse diffs of fixed violations with GumTree in terms of multiple statements.
 * 
 * @author kui.liu
 *
 */
public class FixedViolationHunkParser extends FixedViolationParser {
	...
	private static int thresholdInsertedLines = 3;
	...
	public void parseCodeChangeDiffs(File prevFile, File revFile, File diffentryFile) {
		...
		this.violations = allViolations.size();
		...
		if (this.resultType != 0) {
			...
			for (ViolationInstance v : allViolations) {
			...
			for (ViolationInstance violation : selectedViolations) {
...
//				this.originalTree += Configuration.BUGGY_TREE_TOKEN + "\n" + actionSet.getOriginalTree().toString() + "\n";
	...
	private SimpleTree getBuggyCodeTree(ViolationInstance violation, int bugEndPosition, File prevFile, int bugStartLine, int bugEndLine) {
...
//					// 4. semi-source code: 
	...
	private List<ViolationInstance> identifyFixRangeHeuristically(List<ViolationInstance> violations, List<DiffEntryHunk> diffentryHunks, File revFile) {
							...
							if (line.startsWith("+")) {
								...
								if (currentBugLine == violation.getStartLineNum()) {
									if (bugFixStartLine <= 0) {
										if (fixedLines != 0) {
											if (thresholdInsertedLines != -1 && fixedLines > thresholdInsertedLines) {
												fixedLines = thresholdInsertedLines;// insert 3 lines.
											}
								...
								if (currentBugLine == violation.getEndLineNum()) {
									if (bugFixStartLine != 0 && bugFixEndLine == 0) {
										bugFixEndLine = currentFixLine + (thresholdInsertedLines == -1 ? 0 : thresholdInsertedLines);
