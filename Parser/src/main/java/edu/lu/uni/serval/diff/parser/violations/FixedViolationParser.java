...
import edu.lu.uni.serval.diff.parser.Parser;
...
//			actionSets.addAll(new ActionFilter().filterOutUselessActions(allActionSets));
	...
	/**
	 * Read the code scope of violations, and filter out violations that are starting and ending from -1.
	 * 
	 * @param prevFile
	 * @param revFile
	 * @return
	 */
	protected List<ViolationInstance> readViolations(File prevFile, File revFile) {
		String fileName = revFile.getName();
		List<ViolationInstance> violations = new ArrayList<>();
		String positions = FileHelper.readFile(positionFile);
		BufferedReader reader = null;
		reader = new BufferedReader(new StringReader(positions));
		String line = null;
		try {
			while ((line = reader.readLine()) != null) {
				...
				if (startLine == -1) {
					...
					FileHelper.outputToFile("Exceptions/list1.txt", violationType + ":" + startLine + ":" + endLine + ":" + prevFile + "\n", true);
					continue;
				}
				...
				/*
				 *  0: no source code, 
				 * -1: range is too large, which contains several inner classes, methods or fields.
				 */
				if (violationStartLine > 0) {
...
//					}
	 ...
	 * @return
	 */
	protected String readPatchSourceCode(ViolationInstance violation, int bugStartLine, int bugEndLine, int fixStartLine, int fixEndLine) {
		String patch = "";
