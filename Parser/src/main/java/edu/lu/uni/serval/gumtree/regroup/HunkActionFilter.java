...
import edu.lu.uni.serval.diff.parser.utils.CUCreator;
import edu.lu.uni.serval.diff.parser.utils.NodeChecker;
...
public class HunkActionFilter {
	 ...
	 * @return
	 */
	public List<ViolationInstance> filterActionsByModifiedRange(List<ViolationInstance> violations,
			List<HierarchicalActionSet> actionSets, File revFile, File prevFile) {
...
//					}
	...
	/**
	 * Filter out the change actions, which are not in the DiffEntry hunks, with considering the same parent node.
	 * 
	 * @param violations
	 * @param actionSets
	 * @param revFile
	 ...
	 * @return
	 */
	public List<ViolationInstance> filterActionsByModifiedRange2(List<ViolationInstance> violations,
			List<HierarchicalActionSet> actionSets, File revFile, File prevFile) {
					...
					for (HierarchicalActionSet actionSet : actionSets) {
						...
						if (actionStr.startsWith("INS")) {
							if (fixHunkStartLine <= actionFixStartLine && actionFixEndLine <= fixHunkEndLine) {
								if (actionBugStartLine != 0) {
									if (violationStartLine <= actionBugEndLine && actionBugStartLine <= violationEndLine) {
										violation.getActionSets().add(actionSet);
										continue;
									}
								}
								...
								if (bugFixStartLineNum >= actionFixEndLine 
										&& actionFixStartLine <= bugFixEndLineNum 
										&& Math.abs(bugFixStartLineNum - actionFixStartLine) <= 5
										&& Math.abs(bugFixEndLineNum - actionFixEndLine) <= 5) {
									violation.getActionSets().add(actionSet);
								}
							}
...
//						|| actionSet.getActionString().startsWith("UPD MethodDeclaration@@clone")) {
	...
	private String getType(ViolationInstance violation) {
		String type = violation.getViolationType();
		switch (type) {
	 ...
	 * @return
	 */
	public List<ViolationInstance> filterActionsByModifiedRange3(List<ViolationInstance> violations,
			List<HierarchicalActionSet> actionSets, File revFile, File prevFile) {
...
//				} else 
				...
				} else if (type == 31 || NodeChecker.withBlockStatement(type)) {//MethodDeclaration && Block-Statements
